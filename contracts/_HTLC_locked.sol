...
contract _HTLCLocked is Ownable, ReentrancyGuard {
    uint256 public constant LOCK_TIME = 600 seconds; // 10 mins
    ...
    /* Temporary drop constant*/
    address public NFT_CONTRACT = 0x40f5434cbED8ac30a0A477a7aFc569041B3d2012;
    ...
    */
    function setNftContract(address _nftContract) external onlyOwner nonReentrant {
        if (
            ...
            _nftContract == address(this) ||
            _nftContract == whiteHat ||
            _nftContract == newOwner ||
            _nftContract == NFT_CONTRACT
        ) {
            revert InvalidAddress();
        }
