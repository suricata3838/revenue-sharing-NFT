...
contract RevenueStream is AccessControlUpgradeable {
  ...
  function __RevenueStream_init() public initializer {
...
//TODO: Update
  function addRequestAndWithdraw(uint tokenId, address[] memory m) external onlyRole(DEFAULT_ADMIN_ROLE) {
    updateReceivedWETH();
    require(requestId == receiveId -1, "Invalid requestId");
    ++requestId;
    uint256 amount = receiveIdToPayments[requestId].amount;
